...
class ConvMacroOp(MacroOp):
    def __init__(
        ...
        self.total_ic_num = total_ic_num
    ...
    def emit(self, emitter):
        emitter.set_target(ComponentType.CU)
        cu_config = emitter.cu_config_reg
        ifm_config = {
        ...
        cu_config.ifm_config_reg.emit(emitter, ifm_config)
        ...
        weight_config = {
            ...
            OpCode.WEIGHT_ZERO_POINT: self.weight_zp,
            OpCode.TOTAL_IFM_CHANNEL: self.total_ic_num,
            OpCode.WEIGHT_BASE: self.weight_base,
            # OpCode.WEIGHT_REGION: [
            #     self.weight_region_type,
            # ],
        }
        cu_config.weight_config_reg.emit(emitter, weight_config)
        ...
        cu_config.ofm_config_reg.emit(emitter, ofm_config)
        ...
        cu_config.quantization_config_reg.emit(emitter, quantization_config)
        ...
        cu_config.misc_config_reg.emit(emitter, misc_config)

        emitter.emit_conv(self.accu_en, self.bias_en, self.req_en, self.dep_tag)
